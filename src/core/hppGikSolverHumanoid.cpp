...
ChppGikSolverHumanoid::ChppGikSolverHumanoid(CjrlHumanoidDynamicRobot& inRobot)
{
    attRobot = &inRobot;
...
bool ChppGikSolverHumanoid::weights ( vectorN& inWeights )
{
    ...
    {
        std::cout << "ChppGikSolverHumanoid::weights() incorrect size()\n";
        return false;
    }
}

void ChppGikSolverHumanoid::computeConstraintDofs(CjrlGikStateConstraint* inConstraint)
{
...
bool ChppGikSolverHumanoid::solve ( std::vector<CjrlGikStateConstraint*>& inSortedConstraints)
{
    std::vector<double> SRcoefs(inSortedConstraints.size(), 0);
    return solve(inSortedConstraints, SRcoefs);
}

bool ChppGikSolverHumanoid::solve(std::vector<CjrlGikStateConstraint*>& inSortedConstraints, std::vector<double>& inSRcoefs)
{
    ...
    {
        std::cout << "ChppGikSolverHumanoid::solve() fixed joint not found in robot\n";
        return false;
    }
    ...
    vectorN vals = subrange(attRobot->currentConfiguration(),6,attRobot->numberDof());
    vectorN rate = subrange(attRobot->currentVelocity(),6,attRobot->numberDof());
    attComputationWeights = attWeights;
    attBounder->modifyWeights( vals, rate, attComputationWeights );
    ...
    while ( recompute )
    {
        ...
        {
            paramIndex = iC-6;
            lb = attRobot->lowerBoundDof(iC, CurFullConfig);
            ub = attRobot->upperBoundDof(iC, CurFullConfig);
...
const vectorN& ChppGikSolverHumanoid::solution()
{
    return attSolution;
}

void ChppGikSolverHumanoid::applySolution()
{
    CurFullConfig = attRobot->currentConfiguration();
...
void ChppGikSolverHumanoid::computeFreeFlyerVelocity()
{
...
ChppGikSolverHumanoid::~ChppGikSolverHumanoid()
{
    delete attSolver;
    delete attBounder;
}
