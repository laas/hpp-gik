...
ChppGikPreviewController::ChppGikPreviewController(double inSamplingPeriod)
{
    ...
    if (fabs(inSamplingPeriod-5e-3)<1e-5)
        loadGains(HPP_GIK_GAINDIR "/preview-controller-5ms.ini");
    else
        if (fabs(inSamplingPeriod-1e-2)<1e-5)
            loadGains(HPP_GIK_GAINDIR "/preview-controller-10ms.ini");
        else
            if (fabs(inSamplingPeriod-2e-2)<1e-5)
                loadGains(HPP_GIK_GAINDIR "/preview-controller-20ms.ini");
            else
                if (fabs(inSamplingPeriod-5e-2)<1e-5)
                    loadGains(HPP_GIK_GAINDIR "/preview-controller-50ms.ini");
}
...
bool ChppGikPreviewController::loadGains(const char* inFileName)
{
    ...
    systemA = identity_matrix<double>(3);
